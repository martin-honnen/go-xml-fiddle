      ...
      if (target) {
        ...
        if (target) {
          ...
          if (target) {
            ...
            // We've opened /path/to/somefile.html but the chunk element has an ID,
            // so what's in the ToC is somefile.html#theid. One last try, the hard way:
            let start = path + "#";
            let found = false;
            for (let anchor of document.querySelectorAll("nav.toc div a")) {
