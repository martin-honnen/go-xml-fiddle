...
<xsl:template match="h:nav[contains-token(@class, 'bottom')]">
  <xsl:copy>
    ...
    <xsl:if test="f:is-true($persistent-toc)">
      ...
      <xsl:variable name="toc-elements" as="element()*">
        <xsl:call-template name="tp:resolve-persistent-toc-uris">
          <xsl:with-param name="html" select="/"/>
      ...
      <!-- If there are no entries in the persistent toc; don't do it. -->
      <xsl:if test="exists($toc-elements)">
        <nav class="tocopen"/>
        <nav class="toc"/>
        ...
        <script type="text/html" class="toc">
          <header>
            ...
            <xsl:if test="$persistent-toc-search">
              <p class="ptoc-search">
                <input id="ptoc-search-{f:generate-id(.)}"
                       type="search" class="ptoc-search" placeholder="Search" style="width: 80%"/>
              </p>
            </xsl:if>
          </header>
          <xsl:choose>
            ...
            <xsl:otherwise>
              <div db-prefix="{fp:resolve-persistent-toc-prefix(/, .)}">
                <xsl:sequence select="$toc-elements"/>
              </div>
            </xsl:otherwise>
          </xsl:choose>
        </script>
