...
<xsl:template match="*[@db-chunk]" priority="10">
  ...
  <html db-chunk="{fp:chunk-output-filename(.)}">
    ...
    <xsl:sequence select="(ancestor::*[@lang])[1]/@lang"/>
...
<xsl:template match="h:source/@srcset">
  <!-- There's a lot of duplication here with the handling of @src/@href
       above, but the @srcset attribute has to parsed differently. -->
  <xsl:param name="rootbaseuri" tunnel="yes"/>
  <xsl:param name="chunkbaseuri" tunnel="yes"/>

  <xsl:variable name="srcset" select="."/>

  <xsl:variable name="avalue" as="xs:string*">
    <xsl:for-each select="tokenize(normalize-space(.), ',\s*')">
      <xsl:variable name="uripart"
                    select="if (contains(., ' '))
      ...
      <xsl:variable name="uri" as="xs:string">
        <xsl:choose>
          <xsl:when test="exists(f:uri-scheme($uripart)) and f:uri-scheme($uripart) != 'file'">
            <xsl:sequence select="$uripart"/>
          </xsl:when>
          <xsl:when test="exists($mediaobject-output-base-uri)">
            <xsl:choose>
              <xsl:when test="f:is-true($mediaobject-output-paths)">
              ...
              <xsl:otherwise>
                <xsl:sequence
                    select="fp:relative-uri($rootbaseuri, $chunkbaseuri, '')
                            || $mediaobject-output-base-uri
                            || tokenize($uripart, '/')[last()]"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:when>
          <xsl:otherwise>
            <xsl:sequence
                select="fp:relative-uri($rootbaseuri, $chunkbaseuri, '') || $uripart"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:variable>

      <xsl:variable name="adjusted-uri" as="xs:string">
        <xsl:apply-templates select="$srcset" mode="m:mediaobject-output-adjust">
          <xsl:with-param name="adjusted-uri" select="$uri"/>
        </xsl:apply-templates>
      </xsl:variable>
      ...
      <xsl:sequence select="$adjusted-uri || $therest"/>
    </xsl:for-each>
  </xsl:variable>

  <xsl:attribute name="{local-name(.)}" select="string-join($avalue, ', ')"/>
</xsl:template>
