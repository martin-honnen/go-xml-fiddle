...
<xsl:function name="f:cals-table-fixup" as="element()">
  <xsl:param name="table" as="element()"/>
  <xsl:choose>
    <xsl:when test="$table/@g:preprocessed = 'true'">
      <xsl:sequence select="$table"/>
    </xsl:when>
    <xsl:otherwise>
      <xsl:apply-templates select="$table" mode="m:calstable"/>
    </xsl:otherwise>
  </xsl:choose>
</xsl:function>

<xsl:template name="t:cals-table-fixup" as="element()">
  <xsl:param name="table" as="element()"/>
  <xsl:choose>
    <xsl:when test="$table/@g:preprocessed = 'true'">
      <xsl:sequence select="$table"/>
